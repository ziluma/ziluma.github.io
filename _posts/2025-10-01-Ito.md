...
We present an oversimlified story about the Ito formula, taken from my lecture notes on math finance.
...
where $\pi=\{0=t_0<\cdots<t_N=T\},|\pi|=\max |t_{i}-t_{i-1}|.$
...
Given a partition $\pi=\lbrace 0=t_1<\cdots t_N=T\rbrace$ and write $\delta_i=t_i-t_{i-1},\Delta_i=W_{t_i}-W_{t_{i-1}}\sim\mathcal{N}(0,\delta_i),$
